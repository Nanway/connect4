...
class Minimax(Agent):
    ...
    def act(self, state, game):
        # convert state into correct format
        state = np.squeeze(state)
        state = np.flip(state, axis=0).astype(int)
        bestMove = self.bestMove(4, state.tolist(), self.player)
        return bestMove[0]
    ...
    def bestMove(self, depth, state, curr_player):
        """ Returns the best move (as a column number) and the associated alpha
            Calls search()
            Expects state as 
            [ bottom row, next bottom row, next next bottom row]
        """
